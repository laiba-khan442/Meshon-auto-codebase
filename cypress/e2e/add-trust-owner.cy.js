//Navigates to settings and creates a company type owner.
...
describe('It navigates to settings and creates a company type owner', () => {
 it('passes', () => {
    ...
    cy.get("#entity_type_select", {timeout: 5000})
      .should("be.visible")
      .select("Trust", {force: true}); 
    cy.wait(1000);
    cy.get('#trust_name').type("Test trust");
    const rand = getRandomIntegerUpTo(100); 
    cy.get("#email").type("Test.trust" + rand + "@gmail.com"); 
    ...
    cy.get("#field-container > div > div:nth-child(1) > div > div > div.icheck-danger.d-inline.mr-2 > label").click();
    cy.wait(1000);
    cy.get("#trustee_company", {timeout: 5000})
        .should("be.visible")
        .select("2", {force: true}); 
    cy.wait(1000); 
    cy.get("#trustee_contact_person", {timeout: 5000})
        .should("be.visible")
        .select("4", {force: true}); 
    cy.get("#crn").type("123456789"); 
    cy.get("#abn").type("123456789"); 
    cy.get("#field-container > div > div:nth-child(7) > div > div > div:nth-child(1) > label").click(); 
